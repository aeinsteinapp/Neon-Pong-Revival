# DeadmanXXXII's Classic Pong - Neon + Music + Glow Effects
# Requires: pip install kivy
# Place '8-bit-loop-music-290770.mp3' in the same folder as this script
# Music/SFX for hit/over are included as base64-encoded samples below
...
# ------------------------- Helper: save/load -------------------------
def load_settings():
    default = {"difficulty": "Normal", "sound_enabled": True, "music_volume": 0.5, "sfx_volume": 0.7}
    if os.path.exists(SETTINGS_FILE):
        try:
            with open(SETTINGS_FILE, "r") as f: parsed = json.load(f)
            for k in default: parsed.setdefault(k, default[k])
            return parsed
...
def load_highscore():
    if os.path.exists(HIGHSCORE_FILE):
        try:
            with open(HIGHSCORE_FILE, "r") as f: return json.load(f).get("high_score",0)
        except: return 0
    return 0

def save_highscore(score):
    try:
        with open(HIGHSCORE_FILE, "w") as f: json.dump({"high_score":score}, f)
    except: pass
...
def create_sample_audio():
    for name, b64 in [("hit.wav", hit_wav_b64), ("over.wav", over_wav_b64)]:
        if not os.path.exists(name):
            with open(name, "wb") as f: f.write(base64.b64decode(b64))
...
class PongGame(Widget):
    ...
    def apply_sound_settings(self):
        if self.music:
            ...
            self.music.loop = True  # Ensure looping
            if self.sound_enabled: self.music.play()
    ...
    def serve_ball(self, vel=(4,0)):
        ...
        self.game_over = False
        if self.restart_button: self.remove_widget(self.restart_button)
        if self.win_label: self.remove_widget(self.win_label)

    def update(self, dt):
        if self.game_over: return
        self.ball.move()
        # bounce off top/bottom
        if self.ball.y<0 or self.ball.top>self.height: self.ball.velocity_y*=-1
        # paddle collisions
        ...
        if self.computer.center_y < self.ball.y: self.computer.center_y+=ai_speed
        elif self.computer.center_y>self.ball.y: self.computer.center_y-=ai_speed
        # out of bounds
        if self.ball.x<self.x: self.computer.score+=1; self.serve_ball((4,0))
        if self.ball.right>self.width: self.player.score+=1; self.serve_ball((-4,0))
        # neon glow drawing
        ...
        # check win
        if self.player.score>=10 or self.computer.score>=10: self.end_game()
        # neon title
        for c in list(self.children):
            if hasattr(c,"neon_title") and c.neon_title: self.remove_widget(c)
        neon = Label(text="[b][color=#00FFFF]DeadmanXXXII's[/color] [color=#FF00FF]Classic Pong[/color][/b]",
    ...
    def end_game(self):
        self.game_over=True
        if self.sound_enabled and self.sfx_over: self.sfx_over.play()
        if self.player.score>self.high_score:
            self.high_score=self.player.score
            save_highscore(self.high_score)
        winner="You Win!" if self.player.score>self.computer.score else "You Lose"
        self.win_label=Label(text=f"{winner}\nScore:{self.player.score}\nHigh:{self.high_score}",
                             font_size="20sp",halign="center",valign="middle",
                             size_hint=(None,None),size=(300,120),pos=(self.center_x-150,self.center_y-20))
        self.add_widget(self.win_label)
        self.restart_button=Button(text="Restart",size_hint=(0.4,0.12),
    ...
    def on_touch_move(self, touch):
        if not self.game_over and touch.x < self.width/3: self.player.center_y = touch.y
    def stop_music(self):
...
class MenuScreen(Screen):
    def on_pre_enter(self,*a):
        ...
        title=Label(text="[b][color=#00FFFF]DeadmanXXXII's[/color]\\n[color=#FF00FF]Classic Pong[/color][/b]",
...
class SettingsScreen(Screen):
    def on_pre_enter(self,*a):
        ...
        title=Label(text="Settings",font_size='28sp',size_hint=(1,0.2))
        # difficulty
        diff_label=Label(text=f"Difficulty: {app.difficulty}",size_hint=(1,0.1))
        diff_btn=Button(text="Change Difficulty",size_hint=(1,0.12))
        diff_btn.bind(on_press=lambda x:self.change_diff(diff_label))
        # sound
        sound_label=Label(text=f"Sound: {'On' if app.sound_enabled else 'Off'}",size_hint=(1,0.1))
    ...
    def change_diff(self,label):
        ...
        idx=diffs.index(app.difficulty) if app.difficulty in diffs else 1
        app.difficulty=diffs[(idx+1)%len(diffs)]
        label.text=f"Difficulty: {app.difficulty}"
        settings=load_settings(); settings["difficulty"]=app.difficulty; save_settings(settings)
    def toggle_sound(self,label):
        app=App.get_running_app()
        app.sound_enabled=not app.sound_enabled
        label.text=f"Sound: {'On' if app.sound_enabled else 'Off'}"
        settings=load_settings(); settings["sound_enabled"]=app.sound_enabled; save_settings(settings)
